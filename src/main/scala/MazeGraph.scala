...
object MazeGraph {
    ...
    def mazeToGraph(maze: Array[Array[Int]]): mutable.Map[Node, List[(Node, Int)]] = {
        ...
        //loop through each row and column, and only process walkable cells (1 = walkable, 0 = wall)
        for (row <- maze.indices; col <- maze(0).indices if maze(row)(col) == 1) {
            val node = Node(row, col)
            //find neighboring nodes
            val neighbors = directions.flatMap {
                case (direction_r, direction_c) =>
